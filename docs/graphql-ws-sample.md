...
React + TypeScript + Vite（フロント）と、Node.js スタブ（バックエンド）で  
...
## 1. バックエンド（Node.js Stub）

バックエンドのセットアップとコード全文は `docs/graphql-ws-stub.md` を参照してください。  
WS エンドポイントは `ws://localhost:4000/graphql` で起動します。
...
export const MessageList: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      >
        {messages.length === 0 && (
          <div style={{ color: '#777' }}>まだメッセージはありません。</div>
        )}
        {messages.map((m) => (
          <div
            key={m.id}
